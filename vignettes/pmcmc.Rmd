...
simulate_detSIR<-function(betatau,N,times){
  ...
  traj <- simulate_model(model = SIR_ode,
                       sim_fn = solve_ode,
                       inits = inits,
                       params = parameters,
                       times = times)
...
times <- seq(0, 10, 0.1)
traj<-simulate_detSIR(c(3,1),1000,times)
...
logl_detSIR<-function(parms,C_n){
  ...
  tau<-parms[2]
  p<-parms[3]
  traj<-simulate_detSIR(c(beta,tau),1000,times)
  c_n<-getc_n(traj,T_n)
...
3. Run SMC with the proposed parameters $\theta_{m-1}$ to approximate the marginal likelihood $\mathcal L(\theta_{m})$ :
    ...
    2. For $n=1,2,...,N$, draw the current states $x_n^{(j)}$ by simulating the stochastic SIR model (characterised by $\theta_m$) with the corresponding previous state $x_{n-1}^{(j)}$.
    3. Calculate the weights for each particle $x_n^{(j)}$ as $w_n^{(j)}=\frac{g(y_n;x_n^{(j)})}{\sum_{j=1}^J g(y_n;x_n^{(j)})}$ (resample the particles/trajectories if necessary).
    4. Repeat 2-3 to obtain the approximated marginal likelihood as $\hat L(\theta_m)=\sum_{n=1}^N\sum_{j=1}^Jw_n^{(j)}g(y_n;x_n^{(j)})$
    
4. Accept/reject the proposed parameters $\theta_{m-1}$ based on the comparison between $\hat L(\theta_m)$ and $\hat L(\theta_m)$
