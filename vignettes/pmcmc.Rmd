...
## Introduction
This vignette is to introduce to the {idmodelr} package users a simple use example of the particle Markov-chain Monte Carlo (PMCMC) algorithm. We take simple deterministic/stochastic Susceptible-Infectious-Recovered (SIR) models with partial observation as model cases and show how the models are coupled with PMCMC for efficient parameter estimation.
