...
library(purrr);library(tibble);library(dplyr);library(matrixStats)
```
...
sim_outbreak<-function(betatau,init,times,model=SIR_ode,sim_fn=simulate_deSolve){
...
traj<-sim_outbreak(c(3,1),c(999,1,0),times,model=SIR_stoch_c,sim_fn=simulate_GA)
# plot the epicurve
plot_model(traj, facet = F)
```
...
simulateSIR<-function(parms,starttime, endtime, init){
  outbreak<-sim_outbreak(parms,init,c(starttime,endtime),model=SIR_stoch_c,sim_fn=simulate_GA)
...
smc<-function(parms,J){
  ...
  if(is.na(sum(logweight)))logweight=numeric(J)
