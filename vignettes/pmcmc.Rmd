...
simulate_detSIR<-function(betatau,N,times){
  parameters <- data.frame(
    beta = betatau[1], ##Transmission rate = contact rate * transmission probablity
    tau = betatau[2] ## Rate recovcery = 1 / duration of infection
  )
  inits <- data.frame(
    S = N-1,
    I = 1,
...
C_n<-c(3,18,91,96,42,17,7,2,1,1) # Observed case counts
...
optim(numeric(3)+0.5,logl_detSIR,lower=numeric(3)+0.1,upper=c(5,5,1),method="L-BFGS",C_n=C_n,control = list(fnscale=-1))
```
...
parms=c(1,1,0.5)
for(i in 1:n_iter){
  parms=parms+rnorm(3,0,0.1)
  parms=interval(parms,0,1)
